...
{
    private static Dictionary<VertexKey, List<VertexEntry>> vertexData = new Dictionary<VertexKey, List<VertexEntry>>();
    public static void RecalculateNormalsWithStoredResults(this Mesh mesh, float angle)
    {
        ...
        for (var subMeshIndex = 0; subMeshIndex < mesh.subMeshCount; ++subMeshIndex)
        {
            ...
            for (var i = 0; i < triangles.Length; i += 3)
            {
                ...
                if (!vertexData.TryGetValue(key = new VertexKey(vertices[i1]), out entry))
                {
                    entry = new List<VertexEntry>(4);
                    vertexData.Add(key, entry);
                }
                entry.Add(new VertexEntry(subMeshIndex, triIndex, i1));

                if (!vertexData.TryGetValue(key = new VertexKey(vertices[i2]), out entry))
                {
                ...
                if (!vertexData.TryGetValue(key = new VertexKey(vertices[i3]), out entry))
                {
                    entry = new List<VertexEntry>();
                    vertexData.Add(key, entry);
                }
        ...
        foreach (var vertList in vertexData.Values)
