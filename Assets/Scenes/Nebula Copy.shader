Shader "ShaderMan/Nebula Copy"
{
	Properties{
		_MainTex("MainTex",2D) = "white"{} 
		_StarColor("StarColor",Color) = (0.7,0.8,1.0)
		_Absorption("Absorption", float) = 0.1
		_Scattering("Scattering", float) = 0.5
		_Density("Density", float) = 4.0
	...
	//static  float3 _StarColor = float3(0.7,0.8,1.0);
	float3 _StarColor;
	float _Absorption = 0.1;
	float _Scattering = 0.5;
	float _Density = 4.0;
	float _Radius = 1.0;
	float3 iMouse;
	//static  float _Absorption = 0.1;
	//static  float _Scattering = 0.5;
	//static  float _Density = 4.0;
	//static  float _Radius = 1.0;
...
const float4x4 _Bayer4x4 = float4x4(float4(0,0.5,0.125,0.625),
...
{
	//float2 iMouse = 0;
    float zoom = 0;//iMouse.y/iResolution.y;
